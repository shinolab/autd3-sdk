...
module sim_emission_focus ();
  ...
  logic signed [17:0] focus_x[params::NumBanks][SIZE];
  logic signed [17:0] focus_y[params::NumBanks][SIZE];
  logic signed [17:0] focus_z[params::NumBanks][SIZE];
  logic [7:0] intensity_buf[params::NumBanks][SIZE];
  ...
  task automatic check(input logic bank);
    ...
    for (int j = 0; j < cycle_buf[bank] * freq_div_buf[bank]; j++) begin
      ...
      for (int id = 0; idx < DEPTH; id++) begin
        ...
        x = focus_x[bank][debug_idx_buf] - int'(10.16 * ix / 0.025);  // [0.025mm]
        y = focus_y[bank][debug_idx_buf] - int'(10.16 * iy / 0.025);  // [0.025mm]
        z = focus_z[bank][debug_idx_buf];  // [0.025mm]
        ...
        `ASSERT_EQ(intensity_buf[bank][debug_idx_buf], intensity);
  ...
  initial begin
    ...
    for (int bank = 0; bank < params::NumBanks; bank++) begin
      for (int i = 0; i < SIZE; i++) begin
        focus_x[bank][i] = sim_helper_random.range(131071, -131072 + 6908);
        focus_y[bank][i] = sim_helper_random.range(131071, -131072 + 5283);
