...
module sim_emission_timer ();
  ...
  for (genvar i = 0; i < params::NumBanks; i++) begin
    assign expect_idx[i] = ((sys_time - 2 * DivLatency - 2) / 512 / pattern_settings.FREQ_DIV[i]) % (pattern_settings.CYCLE[i] + 1);
  end

  task automatic check(int bank);
    automatic logic [15:0] idx_old;
    idx_old = idx[bank];
    for (int i = 0; i < pattern_settings.CYCLE[bank] + 10; i++) begin
      while (1) begin
        @(posedge CLK);
      ...
      $display("Check[%1d] %d/%d...done", bank, i + 1, pattern_settings.CYCLE[bank] + 1);
    end
  endtask
  ...
  initial begin
    ...
    pattern_settings.CYCLE[0] = 65536 - 1;
    pattern_settings.FREQ_DIV[0] = 1;
    pattern_settings.CYCLE[1] = 1000 - 1;
