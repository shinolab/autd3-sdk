...
module sim_helper_flash #(
  ...
  always @(posedge CS_N) begin
    if (phase == COMPLETE && opcode == 8'h06) begin
      wel = 1'b1;
    end else if (phase == COMPLETE && opcode == 8'hD8) begin
      if (!wel) begin
        ...
        for (int unsigned i = addr & ~32'hFFFF; i < (addr & ~32'hFFFF) + 32'h10000; i++) begin
          mem.delete(i);
        end
        erases++;
      end
