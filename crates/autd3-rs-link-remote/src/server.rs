...
impl RemoteServer {
    ...
    fn handle_client(&mut self, mut stream: TcpStream) -> Result<(), RemoteLinkError> {
        ...
        let n =
            u16::try_from(self.num_devices).map_err(|_| RemoteLinkError::InvalidDeviceCount {
                found: self.num_devices,
            })?;
        stream.write_all(&n.to_le_bytes())?;
        stream.flush()?;

        let mut tx_buf = vec![0u8; self.num_devices * TX_FRAME_BYTES];
        let mut rx_buf = vec![0u8; self.num_devices * RX_FRAME_BYTES];
        ...
        loop {
            ...
            match tag[0] {
                wire::TAG_FRAME => {
                    ...
                    let rx_valid = self.shared.exchange(&tx_buf, &mut rx_buf)?;
