...
sidebar:
  order: 17
slug: 0.4.x/api/command/lowlevel/change-modulation-bank
---
...
`Gpio(GpioIn)` の詳細は [GPIO ピン](/autd3-sdk/0.4.x/hardware/gpio/) を参照.

同一バンクへの切り替えでは, `TransitionMode` や `LoopBehavior` の設定は機能しない.
バンク運用の詳細は [バンクの切り替え チュートリアル](/autd3-sdk/0.4.x/getting-started/tutorial/bank/) を参照.
