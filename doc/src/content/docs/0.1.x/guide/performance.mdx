...
[Pattern](/autd3-sdk/0.1.x/api/command/pattern/) は内部で `WritePatternBuffer` + `ConfigPattern` + `ChangePatternBank` をまとめて送る複合コマンド.
だが, いったん [バンク](/autd3-sdk/0.1.x/getting-started/tutorial/bank/)を設定してしまえば, 毎フレームの焦点更新で必要なのはバッファの書き換え, すなわち `WritePatternBuffer` だけである.
...
上のホットループでは, 応答を待たずに次々と送る[streaming](/autd3-sdk/0.1.x/getting-started/tutorial/send-modes/)を使っている.
