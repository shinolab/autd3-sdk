...
sidebar:
  order: 3
slug: 0.1.x/api/command/pattern-stm
---

import CodeTabs from "@components/CodeTabs.astro";

`PatternStm` は, ハードウェアタイマにより任意の音場 (各振動子の emission) を切り替える STM 機能を提供する.
[FociStm](/autd3-sdk/0.1.x/api/command/foci-stm/) と異なり, 任意の Pattern を扱えるが, データ転送量が大きい.

## API

<CodeTabs rust={"PatternStm::new(freq, &patterns, option);"} python={"PatternStm(freq, patterns, option)"} csharp={"new PatternStm(new StmConfig(freq), patterns, option);"} />
...
`PatternStm` は内部で [WritePatternBuffer](/autd3-sdk/0.1.x/api/command/lowlevel/write-pattern-buffer/)  (各 index) + [ConfigPattern](/autd3-sdk/0.1.x/api/command/lowlevel/config-pattern/) + [ChangePatternBank](/autd3-sdk/0.1.x/api/command/lowlevel/change-pattern-bank/) の 3 つへ展開される.
以下は `PatternStm::new(freq, &patterns, option)` と等価な組み合わせである.

`PatternStmMode` が `PhaseFull` / `PhaseHalf` のときは, `WritePatternBuffer` の代わりに [WritePatternCompressed](/autd3-sdk/0.1.x/api/command/lowlevel/write-pattern-compressed/) が使われる.
