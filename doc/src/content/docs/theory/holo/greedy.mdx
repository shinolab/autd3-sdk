...
しかも, かなり低い量子化深度(<Math tex="\sim\SI{4}{bit}" />)で十分である ([位相, 及び, 振幅の必要分解能](/autd3-sdk/theory/resolution/)参照).
...
1. <Math tex="j=0, ..., N-1" />まで以下を繰り返す
    1. <Math tex="k=0, ..., K-1" />まで以下を繰り返す
        - <Math tex="l=0, ..., L-1" />まで以下を繰り返す
            - <Math tex="E_j^{kl} = E(q_0, ...,q_{j-1}, a_j^k\rme^{-\im\phi_{j}^l})" />を計算する.
    1. <Math tex="q_j \leftarrow a_j^{k^*}\rme^{-\im\phi_{j}^{l^*}}" />としてセットする. ここで, <Math tex="l^*" />は<Math tex="E_j^{k^*l^*} = \min{\{E_j^{kl}\}}" />を満たす.
