...
\end{aligned}
`} />
この<Math tex="B" />の意味は[固有値問題と行列方程式](/autd3-sdk/theory/holo/evp/)の行列<Math tex="X" />とほとんど同じである.
この<Math tex="B" />を用いて, GS-PATのアルゴリズムは次のように表される.

1. 初期化: <Math tex="\bp^{(0)} = (|p_0|, ..., |p_{M-1}|)\trans" />
1. 以下を<Math tex="n" />回繰り返す:
    1. <Math tex="\bgamma^{(n)} = GB\bp^{(n-1)}" />
    1. <Math tex="\bp^{(n)} = [p^{(n)}(\br_0), ..., p^{(n)}(\br_{M-1})], p_i^{(n)} =\frac{\gamma_i^{(n)}}{\left|\gamma_i^{(n)}\right|}\left|p_i^{(0)}\right|" />
