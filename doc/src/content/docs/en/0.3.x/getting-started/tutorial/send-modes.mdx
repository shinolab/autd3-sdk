...
sidebar:
  order: 9
slug: en/0.3.x/getting-started/tutorial/send-modes
---
...
:::note
For simplicity, both of the above snippets use [`Pattern`](/autd3-sdk/en/0.3.x/api/command/pattern/), a composite command, but internally this decomposes into three low-level commands.
If you merely want to rewrite the output sound field, you should use the low-level command [`WritePatternBuffer`](/autd3-sdk/en/0.3.x/api/command/lowlevel/write-pattern-buffer/).
...
For details on `send` / `send_checked`, refer to [Client](/autd3-sdk/en/0.3.x/api/basics/client/).
