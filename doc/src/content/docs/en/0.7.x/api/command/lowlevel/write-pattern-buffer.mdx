...
sidebar:
  order: 12
slug: en/0.7.x/api/command/lowlevel/write-pattern-buffer
---
...
The following is an example of playing a single Pattern via `WritePatternBuffer` → `ConfigPattern` → `ChangePatternBank` (equivalent to the high-level [Pattern](/autd3-sdk/en/0.7.x/api/command/pattern/)).
