...
sidebar:
  order: 5
slug: en/0.6.x/guide/custom
---

import CodeTabs from "@components/CodeTabs.astro";

`Pattern` and `Modulation` are commands that send an already-computed buffer directly to the device.
The [compute helpers](/autd3-sdk/en/0.6.x/api/compute/pattern/) are pre-provided compute helpers.
...
The following is an example of manually computing the same single focal point as [`focus`](/autd3-sdk/en/0.6.x/api/compute/pattern/focus/), without using a helper.
...
Modulation is a pair of a `&[u8]` buffer representing the AM waveform and a [`SamplingConfig`](/autd3-sdk/en/0.6.x/api/basics/sampling-config/).
