...
Open the newest [`appliance-v*` release](https://github.com/shinolab/autd3-sdk/releases?q=appliance-v&expanded=true) and download `autd3-appliance-rp4-<version>-<date>.img.xz`.
...
The image configures itself, and its root filesystem boots read-only, so the customisation is either ignored or may stop the board from booting.
:::
...
Insert the card and power the board on: the first boot creates the settings area in the free space on the card and reboots on its own.
That takes about a minute.

Discover the appliance from the host to confirm it came up.

```sh
autd3-appliance list
```
...
A Linux host needs a connection profile that holds a link-local address.

## Dashboard

`http://autd3-<serial no.>.local:8081/` shows the state of the bus.
(The address is in the discovery result.)
...
That survives a reboot, and the `uplink` line of `status` reads `radio blocked`.

Both are refused unless the board is reachable over Ethernet, because dropping Wi-Fi while it is the only path leaves the board out of reach.
Add `--force` to go ahead anyway.

`wifi set` brings the radio back even after `--radio-off`.

### When the API can no longer undo it

If `wifi forget --force` left the board unreachable, or neither Ethernet nor Wi-Fi comes up, pull the SD card and fix it there.

Insert the card into a host, and add this line at the end of `config.txt` on the boot partition to stop the radio for good.
