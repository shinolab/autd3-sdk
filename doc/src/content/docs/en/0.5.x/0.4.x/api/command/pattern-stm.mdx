...
sidebar:
  order: 3
slug: 0.5.x/en/0.4.x/api/command/pattern-stm
---
...
Unlike [FociStm](/autd3-sdk/en/0.5.x/0.4.x/api/command/foci-stm/), it can handle arbitrary Patterns, but its data transfer volume is large.
...
`PatternStm` internally expands into the three commands [WritePatternBuffer](/autd3-sdk/en/0.5.x/0.4.x/api/command/lowlevel/write-pattern-buffer/)  (each index) + [ConfigPattern](/autd3-sdk/en/0.5.x/0.4.x/api/command/lowlevel/config-pattern/) + [ChangePatternBank](/autd3-sdk/en/0.5.x/0.4.x/api/command/lowlevel/change-pattern-bank/).
The following is equivalent to `PatternStm::new(freq, &patterns, option)`.

When `PatternStmMode` is `PhaseFull` / `PhaseHalf`, [WritePatternCompressed](/autd3-sdk/en/0.5.x/0.4.x/api/command/lowlevel/write-pattern-compressed/) is used instead of `WritePatternBuffer`.
