...
slug: en/0.5.x/api/compute/holo/naive
---
...
<CodeTabs rust={"naive(\n    &NalgebraBackend,\n    &geometry,\n    &foci,\n    wavelength,\n    &option,\n    &mut dst,\n)?;"} python={"naive(geometry, foci, wavelength, option, dst)"} csharp={"Holo.Naive(geometry, foci, wavelength, option, dst);"} />
...
For the common elements (`ControlPoint` / `TransducerMask` / `LinAlgBackend`, etc.), see the [Holo overview](/autd3-sdk/en/0.5.x/api/compute/holo/).
...
For the meaning of each field, see the [Holo overview](/autd3-sdk/en/0.5.x/api/compute/holo/).
