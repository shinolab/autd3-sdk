...
sidebar:
  order: 4
slug: 0.5.x/en/0.3.x/guide/performance
---
...
[Pattern](/autd3-sdk/en/0.5.x/0.3.x/api/command/pattern/) is a composite command that internally sends `WritePatternBuffer` + `ConfigPattern` + `ChangePatternBank` together.
However, once the [bank](/autd3-sdk/en/0.5.x/0.3.x/getting-started/tutorial/bank/) is set, all that is needed for a per-frame focal point update is rewriting the buffer, that is, only `WritePatternBuffer`.
...
The hot loop above uses [streaming](/autd3-sdk/en/0.5.x/0.3.x/getting-started/tutorial/send-modes/), which sends one after another without waiting for a response.
