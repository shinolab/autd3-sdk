...
sidebar:
  order: 4
slug: en/0.5.x/getting-started/tutorial/moving-focus
---
...
The code that achieves behavior equivalent to the above using [`FociStm`](/autd3-sdk/en/0.5.x/api/command/foci-stm/) is as follows.
...
However, due to constraints of this timer, there are [frequencies that cannot be output](/autd3-sdk/en/0.5.x/api/basics/sampling-config/#internal-representation).
...
Note that by using [`SamplingConfig`](/autd3-sdk/en/0.5.x/api/basics/sampling-config/), the frequency or period can be specified as the sampling frequency, that is per focal point, rather than per loop.

To switch an arbitrary Pattern rather than just a focal point, use [`PatternStm`](/autd3-sdk/en/0.5.x/api/command/pattern-stm/) instead of `FociStm`.
