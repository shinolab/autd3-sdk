...
sidebar:
  order: 4
slug: 0.4.x/en/0.3.x/api/command/foci-stm
---

import CodeTabs from "@components/CodeTabs.astro";

`FociStm` provides an STM feature that switches single- and multi-focus sound fields by a hardware timer.
Unlike [`PatternStm`](/autd3-sdk/en/0.4.x/0.3.x/api/command/pattern-stm/), it can only handle focal points, but its data transfer volume is small, so its latency is low.
...
`FociStm` internally expands into the three commands [WriteFociBuffer](/autd3-sdk/en/0.4.x/0.3.x/api/command/lowlevel/write-foci-buffer/) + [ConfigFociStm](/autd3-sdk/en/0.4.x/0.3.x/api/command/lowlevel/config-foci-stm/) + [ChangePatternBank](/autd3-sdk/en/0.4.x/0.3.x/api/command/lowlevel/change-pattern-bank/).
