...
sidebar:
  order: 4
slug: en/0.9.x/guide/performance
---
...
[Pattern](/autd3-sdk/en/0.9.x/api/command/pattern/) is a composite command that performs the buffer write, the playback configuration and the bank switch together, but these are fused into a single frame.
...
However, once the [bank](/autd3-sdk/en/0.9.x/getting-started/tutorial/bank/) is set, all that is needed for a per-frame focal point update is rewriting the buffer, that is, only `WritePatternBuffer`.
...
The hot loop above uses [streaming](/autd3-sdk/en/0.9.x/getting-started/tutorial/send-modes/), which sends one after another without waiting for a response.
