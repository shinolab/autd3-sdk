...
sidebar:
  order: 2
slug: en/0.8.x/api/command/modulation
---
...
`config` is specified with [`SamplingConfig`](/autd3-sdk/en/0.8.x/api/basics/sampling-config/).
`data` is written to a buffer allocated by `modulation_buffer()`.
For the behavior of `loop_behavior` / `transition_mode`, see [ChangeModulationBank](/autd3-sdk/en/0.8.x/api/command/lowlevel/change-modulation-bank/).

Several [helper functions](/autd3-sdk/en/0.8.x/api/compute/modulation/) are provided for computing modulation.

## Equivalent Low-Level Commands

`Modulation` is equivalent to the three commands [WriteModulationBuffer](/autd3-sdk/en/0.8.x/api/command/lowlevel/write-modulation-buffer/) + [ConfigModulation](/autd3-sdk/en/0.8.x/api/command/lowlevel/config-modulation/) + [ChangeModulationBank](/autd3-sdk/en/0.8.x/api/command/lowlevel/change-modulation-bank/). When the data fits in a single frame it is fused and sent as one frame; otherwise it expands into the three commands above.
