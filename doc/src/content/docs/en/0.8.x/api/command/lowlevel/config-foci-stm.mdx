...
sidebar:
  order: 19
slug: en/0.8.x/api/command/lowlevel/config-foci-stm
---
...
To emit a single focal point statically, use [Pattern](/autd3-sdk/en/0.8.x/api/command/pattern/) instead of an STM.

For the timing at which `loop_behavior` and the transition take effect see [ChangePatternBank](/autd3-sdk/en/0.8.x/api/command/lowlevel/change-pattern-bank/).
